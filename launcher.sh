...
launch_troll_slightly_lock(){
    if $GUM confirm "Are you really sure to install 🔒 Lock your friend's screen" </dev/tty; then
        ft_lock
    fi
}
launch_troll_slightly_matrix(){
    if $GUM confirm "Are you really sure to install 🎬 Lowcost Matrix" </dev/tty; then
        echo 'cat /dev/urandom' >> ~/.zshrc
    fi
}
launch_troll_slightly_tacorcat(){
    if $GUM confirm "Are you really sure to install 🐱 Tac or Cat ?" </dev/tty; then
        echo 'alias cat="rev"' >> ~/.zshrc
    fi
}
launch_troll_slightly_manual(){
    if $GUM confirm "Are you really sure to install 📖 Where is the fucking manual ?" </dev/tty; then
        echo 'man() { echo "No manual entry for $1"; }' >> ~/.zshrc
    fi
}
launch_troll_slightly_hieroglyph(){
    if $GUM confirm "Are you really sure to install 🥸 Hieroglyph Title Bar" </dev/tty; then
...
launch_troll_slightly_cursor(){
    if $GUM confirm "Are you really sure to install 🖱️ Cursor Finder" </dev/tty; then
        gsettings set org.gnome.desktop.interface cursor-size 256
    fi
}
launch_troll_slightly_xxx(){
    if $GUM confirm "Are you really sure to install ❌ XXX ?" </dev/tty; then
        gsettings set org.gnome.desktop.wm.preferences button-layout ':close,close,close'
    fi
}
launch_troll_slightly_useless(){
    if $GUM confirm "Are you really sure to install 🙅 Useless window" </dev/tty; then
        gsettings set org.gnome.desktop.wm.preferences button-layout ':'
    fi
}
launch_troll_moderately_adsh(){
    if $GUM confirm "Are you really sure to install 📺 AdSh - Ads in your shell" </dev/tty; then
...
launch_troll_moderately_fallingkey(){
    if $GUM confirm "Are you really sure to install 🍎 FallingKey" </dev/tty; then
...
launch_troll_annoying_rickrolled(){
    if $GUM confirm "Are you really sure to install 🕺 Rickrolled" </dev/tty; then
...
launch_troll_annoying_nolock(){
    if $GUM confirm "Are you really sure to install 🔑 NoLock" </dev/tty; then
...
launch_troll_annoying_randommouse(){
    if $GUM confirm "Are you really sure to install 🎲 Random Mouse" </dev/tty; then
        while true; do xmodmap -e "pointer = $(shuf -e 1 2 3 | paste -sd ' ')" ; sleep 2; done & disown
    fi
}
launch_troll_annoying_bepolite(){
    if $GUM confirm "Are you really sure to install 😇 Be Polite" </dev/tty; then
